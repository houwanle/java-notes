...
上面代码也达到了预期效果，使用CountDownLatch可以让一个或者多个线程等待一批线程完成之后，自己再继续；
...
CompletableFuture.supplyAsync可以用来异步执行一个带返回值的任务，调用completableFuture.get() 会阻塞当前线程，直到任务执行完毕，get方法才会返回。